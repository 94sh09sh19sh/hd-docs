...
> 迭代 3 完成資料層遷移與四個平台機制；迭代 4 完成 AI 輔助內容與護理記錄；迭代 5 完成求助處理完整紀錄、護理師班表與成效基準；迭代 6 完成閒置輪播與檔案匯入；迭代 7 完成部署前的缺陷修正與版本更新的資料保護；迭代 8 完成兩端介面重新設計。
...
| 量測 | Iteration 2 時點 | 迭代 4 完成 | 迭代 5 完成 | 迭代 6 完成 | 迭代 7 完成 |
|---|---|---|---|---|---|
| API 路由 | 43 | 78 | 90 | 107 | 109 |
| 資料表 | 10 | 29 | 36 | 39 | 40 |
| 稽核動作 | 32 | 52 | 66 | 68 | 74 |
| 權限碼 | 9 | 17 | 20 | 21 | 21 |
| 角色 | 3 | 6 | 6 | 6 | 6 |
...
**迭代 8 沒有新增任何環境變數。** 它一個端點都沒加，設定面完全沒動。
...
最近一次執行（2026-09-19，本機 SQLite，迭代 8 完成後）：**八支皆全數通過**
（`verify:iteration8` 不需要後端在跑，它驗的東西全部在前端原始碼與樣式表裡）；
**五支盤點腳本中，零院外連線 0 筆、介面設計基準全部通過、離線可部署性 0 項阻擋**。
介面技術識別仍是既有的 22 筆，清除排在迭代 9；遷移安全的 1 筆是迭代 5 留下的歷史紀錄。
