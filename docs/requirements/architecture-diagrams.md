...
> **0926 修訂中：下面兩張圖尚未依 0926 改版重繪，以文字為準。** 要改的有四處：
> 應用伺服器的標籤由「離線安裝包」改為「Docker 容器・git clone」；後端與 AI 推論端點之間多一格 **AI 閘道**（本系統的容器，不含模型）；
> 過渡期那條線改為「開發機 → SSH 通道 → 實驗室個人容器裡的 AI 閘道 → 實驗室 Ollama」，並標明只在開發環境存在；
> 應用伺服器多一條**建置期**才有的對外連線（git、映像檔、套件），與執行期零連外分開畫。
> 依據見《[部署規範](deployment-spec.md)》v3.0 第 1、3、5 章。
...
3. 院內裝置與資料庫在執行期一條對外連線都沒有。唯一跨出醫院的是過渡期的實驗室 API，AI 呼叫在程式裡強制只送合成資料。**0926**：院內主機在部署與更新時可以連出去（建置期），正式環境設定下選用實驗室會直接拒絕啟動。
...
| 定版 | 日期 | 異動 |
|---|---|---|
| [0923](https://94sh09sh19sh.github.io/hd-docs/0923/requirements/architecture-diagrams/) | 2026-09-23 | 依 0919 改版重繪：應用伺服器改為共用 Windows 工作站（離線安裝包），AI 推論退成另一台機器，機房裡的 MDM 整格與其連線拿掉，臨床數值確定走院方唯讀 API；補上產圖時的兩個坑 |
