...
| 動了什麼 | 圖上怎麼變 |
|---|---|
| 應用主機由 DGX Spark 改為**院內一台專用 Windows 工作站** | 應用伺服器的標籤改成「專用 Windows 工作站・離線安裝包」 |
...
- **應用伺服器與 AI 推論端點是兩台不同的機器**（0919 起）：應用伺服器是院內一台專用的 Windows 工作站，以離線安裝包部署；AI 推論端點在另一台 DGX Spark 上，不由本專案部署（《[部署規範](deployment-spec.md)》DEP-07、DEP-16）。**0916 以前這兩個框畫的是同一台機器的兩個服務**，現在它們真的是兩台。
...
| 機櫃 | 實體主機（應用伺服器為專用 Windows 工作站；AI 推論端點為另一台機器） |
...
| — | **0919 第一次動到圖二的分區與線**：應用伺服器改為院內一台專用 Windows 工作站（離線安裝包），AI 推論端點退成另一台機器，**MDM 整格與那條「註冊／遠端鎖定」的線一起拿掉**，臨床數值來源改標為院方唯讀 API |
