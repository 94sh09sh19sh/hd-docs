...
**2026-09-12 更新，對應迭代 6 完成時的程式。** 0911 依 SRS v2.0 重繪過一次（推播中繼與外部 LLM
兩條院外線拿掉，PostgreSQL 改為 SQLite 單一檔案，即時通知改走院內 SSE 並加上常駐總覽螢幕，
迭代 5 補上營運與排班模組）；這一次補的是**閒置輪播**與**正式主機**。與 0909 版的差異見文末。

**迭代 6 沒有新增任何一台機器、任何一條網路連線。** 輪播併在照護業務模組裡，檔案匯入是臨床資料匯入
底下多一個來源實作，兩者都在既有的院內後端中。因此圖二的分區與線**一條未改**，只把兩處標籤換成
已經確定的事實：應用伺服器標上「DGX Spark・linux/arm64 容器」，院內 AI 伺服器標上「同一台
DGX Spark・不自行部署」——0912 之前這兩處寫的是「128GB・規格待 Q-07」。

會動到圖二的分區與連線的，是硬體或網段真的變了的時候。
...
- 鏽色線只剩一條：過渡期的實驗室 API，程式強制只送合成資料。虛線外框的規則引擎（迭代 7）與對外報表服務（迭代 8）是規劃中、尚未實作的元件。
...
- 主動跨出醫院的只有一條，起點是應用伺服器：過渡期的實驗室 API。院內 AI 伺服器就位、真實資料要進 AI 流程時，這條線就拿掉（[Q-07、Q-08](open-questions.md)）。
- **應用伺服器就是那台 DGX Spark**：正式環境以 `linux/arm64` 容器部署，院內 AI 伺服器是同一台機器上的另一個服務，不由本專案部署（《[部署規範](deployment-spec.md)》DEP-05、DEP-16）。圖上把它們畫成兩個框，是因為它們的職責與維運者不同，不是因為它們是兩台機器。
- SQLite 檔案與應用伺服器在同一台機器的本機磁碟，不放網路磁碟（資料庫使用規範 7.2）；每日以線上備份寫到院內的備份目的地，不得是雲端同步資料夾（8.1）。
- 常駐總覽螢幕是部署必要條件，不是選配：推播拿掉之後，它是唯一保證通報被看見的地方（FR-N12，位置與硬體待 Q-10）。
- HIS／EMR 用另一種顏色與形狀，因為它不是本專案的系統，只讀、不寫回；臨床數值經臨床資料匯入進來，院方給 API 還是 Excel 待 Q-09。MDM 目前是介面層加模擬實作，實際方案待 Q-11。
...
| [0909](https://hackmd.io/@94sh09sh19sh/hd-arch-0909) | 2026-09-09 | 首次定版 |
