...
| `prisma:migrate` 噴 `getaddrinfo ENOTFOUND binaries.prisma.sh` | **Prisma 的執行引擎不在 npm 套件裡**，是安裝套件時由安裝後腳本另外下載的（約 40MB）。只要 `node_modules` 是在沒有成功下載引擎的情況下產生的——離線安裝、跳過安裝後腳本、或從別處複製過來而排除了二進位檔——第一次用到 Prisma 指令時就會現場去下載 | 開發機上：連網重跑一次 `npm install`，確認 `node_modules/@prisma/engines/` 底下有兩個數十 MB 的執行檔。正式環境的解法不同——引擎隨安裝包攜帶並以環境變數指向，見《部署規範》DEP-30 |
...
| 護理端找不到「解除綁定」的按鈕，但裝置管理頁看得到那台平板正綁著人 | 解除的入口掛在「當日排班」清單上，而那份清單依**排班日期**篩選（預設今天）。綁定的排班日期不是今天時，那一列不會出現 | 把排班日期切到那筆綁定所屬的日期。**這是已知缺陷，修正排在迭代 7**（實作規格書 3.15）：會新增一個不依日期的有效綁定清單 |
