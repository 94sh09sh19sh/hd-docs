...
> 表中各迭代的操作步驟分別在[迭代 3](iteration-3.md)、[迭代 4](iteration-4.md)、[迭代 5](iteration-5.md)、[迭代 6](iteration-6.md)、[迭代 7](iteration-7.md) 五本分冊。
...
| `prisma:migrate` 噴 `getaddrinfo ENOTFOUND binaries.prisma.sh` | **Prisma 的執行引擎不在 npm 套件裡**，是安裝套件時由安裝後腳本另外下載的（約 40MB）。只要 `node_modules` 是在沒有成功下載引擎的情況下產生的——離線安裝、跳過安裝後腳本、或從別處複製過來而排除了二進位檔——第一次用到 Prisma 指令時就會現場去下載 | 開發機上：連網重跑一次 `npm install`，確認 `node_modules/@prisma/engines/` 底下有兩個數十 MB 的執行檔。正式環境的解法不同——引擎隨安裝包攜帶，啟動與遷移時以 `PRISMA_ENGINES_DIR` 指向包內那一份（迭代 7 已實作，見《部署規範》DEP-30）。先跑 `npm run check:offline` 盤點這台機器上的引擎與版本 |
...
| [迭代 7 — 擋住上線的缺陷與資料安全](iteration-7.md) | 不依日期的有效綁定清單、跨日提示、指派清單說得出原因、冪等解除、時區與執行帳號、更新前強制備份與驗證還原、遷移指令防呆、AI 端點不通時的四條主線 | `q1`–`q49` |

走的順序建議照迭代編號：後面的分冊會用到前面建立起來的東西（例如迭代 4 要先照迭代 3 分冊開啟 AI 總開關，迭代 6 的第三層要先有迭代 3 的臨床數值）。

**開始任何一本分冊之前，先把這四樣東西準備好**（五本分冊的「前置」都建立在這些上面）：
...
- 迭代 7 §7.6 的更新流程**要先停掉後端才走得完**（`q23`–`q31`），請留一段不會被打斷的時間；另有兩項只有到部署當天（`q22`）或迭代 10（`q38`）才做得到。
