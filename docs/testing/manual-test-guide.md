...
> **本頁是主手冊**：環境建置、兩條主線、離線韌性、迭代 1／2 的細節功能、收工與附錄。迭代 3 之後每個迭代各有一本分冊，見 [§13 分冊索引](#13--迭代-3-10-分冊)。
...
| [迭代 10 — 打包、平板佈建與部署演練](iteration-10.md) | 離線安裝包的建置與五項驗收、服務註冊與啟停、病人端斷網仍開得出畫面、逸出偵測、更新與回退演練、自簽 CA 與外殼 App、路線 A 與路線 B。**約一半的項次要在乾淨機器、實機平板或院內主機上做** | `t1`–`t53` |

走的順序建議照迭代編號：後面的分冊會用到前面建立起來的東西（例如迭代 4 要先照迭代 3 分冊開啟 AI 總開關，迭代 6 的第三層要先有迭代 3 的臨床數值）。

**開始任何一本分冊之前，先把這四樣東西準備好**（八本分冊的「前置」都建立在這些上面）：
