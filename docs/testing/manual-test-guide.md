...
> **本頁是主手冊**：環境建置、兩條主線、離線韌性、迭代 1／2 的細節功能、收工與附錄。迭代 3 之後每個迭代各有一本分冊，見 [§13 分冊索引](#13--迭代-3-13-分冊)。
...
- [ ] `d10` **［病人端］** 回主畫面 → 點畫面最下方那一條紅色的「我需要協助」。
...
| `prisma:migrate` 噴 `getaddrinfo ENOTFOUND binaries.prisma.sh` | **Prisma 的執行引擎不在 npm 套件裡**，是安裝套件時由安裝後腳本另外下載的（約 40MB）。只要 `node_modules` 是在沒有成功下載引擎的情況下產生的——離線安裝、跳過安裝後腳本、或從別處複製過來而排除了二進位檔——第一次用到 Prisma 指令時就會現場去下載 | **0920 已修，舊版才會遇到。** 迭代 7 的 `PRISMA_ENGINES_DIR` 只接上了**後端啟動**；`prisma migrate`、`prisma generate` 是另外開的行程，不經過那一段，而 **Prisma CLI 不讀 `node_modules/@prisma/engines`**——它只看自己的 `node_modules/prisma/` 與 `node_modules/.cache/prisma`，兩邊都沒有就現場下載。現在所有 prisma 指令都經過 `apps/api/scripts/prisma-cli.mjs`，執行前把路徑指好（見《部署規範》DEP-30）。遇到這個錯誤先確認這一步在不在：`npm run check:offline` 會盤點版本與指令有沒有走這一層。若 `node_modules/@prisma/engines/` 底下真的沒有兩個數十 MB 的執行檔，那是安裝沒裝完，連網重跑一次 `npm install` |
| 病人端在本機開了好幾個分頁，結果只有最後一個認得出自己是哪一台平板 | **同一個瀏覽器的所有分頁共用同一份儲存空間**，後佈建的裝置序號與金鑰會蓋掉先佈建的。這是本機測試環境特有的，正式環境一台平板一支 App、各自獨立 | 每一台模擬平板用**不同的瀏覽器設定檔**（或不同瀏覽器、或無痕視窗各開一個），不要在同一個視窗裡開多個分頁 |
| 護理端找不到「解除綁定」的按鈕，但裝置管理頁看得到那台平板正綁著人 | 解除的入口原本只掛在「當日排班」清單上，而那份清單依**排班日期**篩選（預設今天）。綁定的排班日期不是今天時，那一列不會出現 | **迭代 7 已修**（實作規格書 3.15）：改到「裝置管理」的**目前有效綁定**清單解除，那份清單不分日期。主控台也會在有跨日綁定時明白提示並提供跳過去的連結。驗收步驟見[迭代 7 分冊](iteration-7.md) §7.1 |
...
| [迭代 10 — 打包、平板佈建與部署演練](iteration-10.md) | 離線安裝包的建置與五項驗收、服務註冊與啟停、病人端斷網仍開得出畫面、逸出偵測、更新與回退演練、自簽 CA 與外殼 App、路線 A 與路線 B。**約一半的項次要在乾淨機器、實機平板或院內主機上做**（安裝包部分 0926 已撤除，保留為紀錄） | `t1`–`t53` |
| [迭代 11 — 部署改為 Docker ＋ git clone](iteration-11.md) | repo 裡已沒有安裝包、容器設定盤點（含故意改壞看它變紅）、在開發機上只用 Git 與 Docker 從零部署、正式環境設定拒絕 `lab`、`down -v` 刪不掉資料庫、更新與回退演練 | `u1`–`u43` |
| [迭代 12 — 外殼 App 獨立 repo 與整合](iteration-12.md) | 外殼只在一個地方、版本握手與護理端的「外殼版本」欄、`shell-builder` 建 APK 且第二次不重新產生金鑰、病人端 HTTPS 與下載頁、**開發手機**安裝釘選與跳出、調高最低版本與契約不相容、金鑰備份與核對 | `v1`–`v48` |
| [迭代 13 — AI 閘道與實驗室實際測試](iteration-13.md) | 閘道只有三個端點、不管理模型、建置時自我測試；整條路徑在假推論端點上走一次（實際模型記進呼叫紀錄、真實病人被擋、只改 `config.yaml` 換模型與換方法、閘道停掉主線照常）；**實驗室**上的連線、個人容器、真模型跑通與繁中品質紀錄；repo 沒有連線資訊、實驗室沒有留東西；院內那一份閘道 | `w1`–`w47` |

走的順序建議照迭代編號：後面的分冊會用到前面建立起來的東西（例如迭代 4 要先照迭代 3 分冊開啟 AI 總開關，迭代 6 的第三層要先有迭代 3 的臨床數值）。

**開始任何一本分冊之前，先把這四樣東西準備好**（十一本分冊的「前置」都建立在這些上面）：
...
- 迭代 7 §7.6 的更新流程**要先停掉後端才走得完**（`q23`–`q31`），請留一段不會被打斷的時間；另有兩項只有到部署當天（`q22`）或乾淨機器上（`q38`，做法見[迭代 10 分冊](iteration-10.md) `t8`）才做得到。
...
| 定版 | 日期 | 異動 |
|---|---|---|
| [0923](https://94sh09sh19sh.github.io/hd-docs/0923/testing/manual-test-guide/) | 2026-09-23 | 對應到迭代 9：路由表 107 → 126 條、稽核動作 68 → 164 種，§13 改為迭代 3～10 八本分冊的索引；疑難排解補上 Prisma 引擎離線下載失敗、多分頁搶同一份裝置身分、找不到解除綁定入口三項 |
...
| [0909](https://94sh09sh19sh.github.io/hd-docs/0909/testing/manual-test-guide/) | 2026-09-09 | 首次定版 |
